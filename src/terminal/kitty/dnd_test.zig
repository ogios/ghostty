...
test "dnd: unsupported drag icon does not cancel source" {
    ...
    h.clear();
    _ = try h.command("t=o:o=3", "text/uri-list");
...
test "dnd: drag source retries discard partial data" {
    var h: Harness = .init();
    defer h.deinit();
    _ = try h.command("t=o:x=1", null);
    _ = try h.registered().requestDrag(&h.output.writer, .{ .cell_x = 0, .cell_y = 0, .pixel_x = 0, .pixel_y = 0, .operations = .{} });
    _ = try h.command("t=o:o=1", "text/plain text/html");
    _ = try h.command("t=P:x=-1", null);
    try h.registered().sourceStartResult(testing.allocator, &h.output.writer, true);
    _ = try h.registered().requestSourceData(&h.output.writer, 0);
    _ = try h.command("t=e:y=0", "b2xk");
    try testing.expectEqual(dnd.Event.source_data_error, (try h.command("t=E:y=0", "EIO")).?);
    try testing.expect(h.registered().sourceData(0) == null);
    try testing.expectEqual(@as(usize, 0), h.registered().source.total_data_bytes);
    _ = try h.registered().requestSourceData(&h.output.writer, 0);
    _ = try h.command("t=e:y=0", "bmV3");
    _ = try h.command("t=e:y=0", "");
    try testing.expectEqualStrings("new", h.registered().sourceData(0).?);
    try testing.expectEqual(@as(usize, 3), h.registered().source.total_data_bytes);
}

test "dnd: drag source interleaved MIME replies" {
    var h: Harness = .init();
    defer h.deinit();
    _ = try h.command("t=o:x=1", null);
    _ = try h.registered().requestDrag(&h.output.writer, .{ .cell_x = 0, .cell_y = 0, .pixel_x = 0, .pixel_y = 0, .operations = .{} });
    ...
    try h.registered().sourceStartResult(testing.allocator, &h.output.writer, true);
    _ = try h.registered().requestSourceData(&h.output.writer, 0);
    _ = try h.registered().requestSourceData(&h.output.writer, 1);
    _ = try h.command("t=e:y=0", "Zm9v");
    _ = try h.command("t=e:y=1", "YmFy");
    _ = try h.command("t=e:y=0:m=1", "YmF6");
    _ = try h.command("m=0", "cXV4");
    try testing.expect(h.registered().sourceData(0) == null);
    try testing.expect(h.registered().sourceData(1) == null);
    try testing.expectEqual(dnd.Event.source_data, (try h.command("t=e:y=0", "")).?);
    try testing.expectEqual(@as(?usize, 0), h.registered().sourceReadyIndex());
    try testing.expectEqual(dnd.Event.source_data, (try h.command("t=e:y=1", "")).?);
    try testing.expectEqual(@as(?usize, 1), h.registered().sourceReadyIndex());
    try testing.expectEqualStrings("foobazqux", h.registered().sourceData(0).?);
    try testing.expectEqualStrings("bar", h.registered().sourceData(1).?);
    try testing.expectEqual(@as(usize, 12), h.registered().source.total_data_bytes);
    h.clear();
    try testing.expect(!try h.registered().requestSourceData(&h.output.writer, 0));
    try h.expectOutput("");
    try testing.expectEqualStrings("foobazqux", h.registered().sourceData(0).?);
